...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        newMaxQ = self.computeValueFromQValues(nextState)
        self.values[(state, action)] = (1 - self.alpha) * oldValue + self.alpha * (reward + self.discount * newMaxQ)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        # weights = self.getWeights()
        newMaxQ = self.computeValueFromQValues(nextState)
        newVal = reward + self.discount * newMaxQ
        diff = newVal - currVal
        for feature in features:
